  ...
  const renderItem = ({ item }) => (
    ...
    >
      <View style={styles.cardContent}>
        ...
        <View>
          <Text style={styles.cardTitle}>{item.title || `${type} Paper`}</Text>
          <Text style={styles.cardSubtitle}>{item.year ? `Year: ${item.year}` : new Date(item.createdAt?.toDate()).toLocaleDateString()}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: '600', color: '#1b1b1f', textTransform: 'capitalize' },
